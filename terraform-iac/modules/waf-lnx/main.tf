# Create public IPs
resource "azurerm_public_ip" "waf1-nic-private-dmz-puip" {
    depends_on = [var.waf_depend_on]
    name                         = "${var.current-name-convention-core-module}-waf1-ub16-nicinpuip"
    location                     = "${var.preferred-location-module}" 
    resource_group_name          = "${var.current-name-convention-core-module}-rg"
    domain_name_label            = "${var.current-name-convention-core-public-module}waf1"
    allocation_method            = "Static"
    tags = "${var.tags-waf-public-dmz-module}"
}
resource "azurerm_network_interface" "waf1-nic-private-dmz-in" {
  depends_on = [var.waf_depend_on]
  name                 = "${var.current-name-convention-core-module}-waf1-ub16-nicin"
  location            = "${var.preferred-location-module}" 
  resource_group_name = "${var.current-name-convention-core-module}-rg"
  enable_ip_forwarding = true
  ip_configuration {
    name                          = "${var.current-name-convention-core-module}-waf1-ub16-nicinconfig"
    subnet_id                     = "${var.subnet_in_id_module}" 
    private_ip_address_allocation = "Static"
    private_ip_address            = "${var.ip-in-waf-module}"
    public_ip_address_id          = "${azurerm_public_ip.waf1-nic-private-dmz-puip.id}"
  }
  tags = "${var.tags-waf-public-dmz-module}"
}
resource "azurerm_virtual_machine" "waf1-ub16" {
  name                 = "${var.current-name-convention-core-module}-waf1-ub16"
  location            = "${var.preferred-location-module}" 
  resource_group_name = "${var.current-name-convention-core-module}-rg"
  network_interface_ids = ["${azurerm_network_interface.waf1-nic-private-dmz-in.id}"]
  vm_size               = "${var.waf-size}"
  tags = "${var.tags-waf-public-dmz-module}"
  ...
  storage_os_disk {
    name              = "${var.current-name-convention-core-module}-waf1-ub16-disk-os"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Standard_LRS"
  }
  os_profile {
    computer_name  = "${var.current-name-convention-core-module}-waf1-ub16"
    admin_username = "${var.waf-login}"
    admin_password = "${var.waf-passwd}" 
  }
  boot_diagnostics {
        enabled     = "true"
...
resource "azurerm_virtual_machine_extension" "waf1_custom_script" {
  name                 = "${var.current-name-convention-core-module}-waf1-ub16-customscript-nginx"
  virtual_machine_id   = azurerm_virtual_machine.waf1-ub16.id
  publisher            = "Microsoft.Azure.Extensions"
  type                 = "CustomScript"
  type_handler_version = "2.0"
  settings = <<SETTINGS
    {
        "commandToExecute": "apt-get -y update && sudo apt install nginx -y"
    }
SETTINGS
}


resource "azurerm_virtual_machine_extension" "waf1_oms_mma" {
 depends_on = [azurerm_virtual_machine_extension.waf1_custom_script]
 name                 = "${var.current-name-convention-core-module}-waf1-ub16-OMSExtension"
...
resource "azurerm_virtual_machine_extension" "waf1_diag_setting" {
 depends_on = [azurerm_virtual_machine_extension.waf1_custom_script]
 name                 = "${var.current-name-convention-core-module}-waf1-ub16-LinuxDiagnostics"
 virtual_machine_id = "${azurerm_virtual_machine.waf1-ub16.id}"
  ...
  settings = <<SETTINGS
        ...
        "ladCfg": {
            ...
            "metrics": {
                ...
                "resourceId": "${azurerm_virtual_machine.waf1-ub16.id}"
            }, 
            "performanceCounters": {
