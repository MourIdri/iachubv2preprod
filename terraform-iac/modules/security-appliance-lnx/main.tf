resource "azurerm_network_interface" "nva1-nic-private-dmz-in" {
  ...
  ip_configuration {
    name                          = "${var.current-name-convention-core-module}-nva1-ub16-nicinconfig"
...
resource "azurerm_network_interface" "nva1-nic-private-dmz-out" {
  depends_on = [var.security_appliance_depend_on]
  name                 = "${var.current-name-convention-core-module}-nva1-ub16-nicout"
  location            = "${var.preferred-location-module}" 
  resource_group_name = "${var.current-name-convention-core-module}-rg"
  enable_ip_forwarding = true
  ip_configuration {
    name                          = "${var.current-name-convention-core-module}-nva1-ub16-nicoutconfig"
    subnet_id                     = "${var.subnet_out_id_module}" 
    private_ip_address_allocation = "Static"
    private_ip_address            = "${var.ip-out-nva-module}" 
  }
  tags = "${var.tags-security-appliance-dmz-module}"
}
resource "azurerm_virtual_machine" "nva1-ub16" {
  name                 = "${var.current-name-convention-core-module}-nva1-ub16"
  location            = "${var.preferred-location-module}" 
  resource_group_name = "${var.current-name-convention-core-module}-rg"
  network_interface_ids = ["${azurerm_network_interface.nva1-nic-private-dmz-in.id}","${azurerm_network_interface.nva1-nic-private-dmz-out.id}"]
  primary_network_interface_id = "${azurerm_network_interface.nva1-nic-private-dmz-in.id}"
  vm_size               = "${var.nva-size}"
  ...
  storage_os_disk {
    name              = "${var.current-name-convention-core-module}-nva1-ub16-disk-os"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Standard_LRS"
  }
  os_profile {
    computer_name  = "${var.current-name-convention-core-module}-nva1-ub16"
    admin_username = "${var.nva-login}"
    admin_password = "${var.nva-passwd}" 
  }
  boot_diagnostics {
        enabled     = "true"
...
resource "azurerm_virtual_machine_extension" "nva1_custom_script" {
  name                 = "${var.current-name-convention-core-module}-nva1-ub16-customscript-routing"
...
resource "azurerm_virtual_machine_extension" "nva1_oms_mma" {
 name                 = "${var.current-name-convention-core-module}-nva1-ub16-OMSExtension"
